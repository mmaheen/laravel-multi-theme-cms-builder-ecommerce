<section class="mt-5">
    <h1 class="mx-auto text-center text-2xl max-w-2/4">Lorem ipsum dolor sit amet.</h1>
    <p class="mx-auto text-center mb-4 text-slate-500 max-w-2/4">
        Lorem ipsum, dolor sit amet consectetur adipisicing elit. Odio quis doloremque doloribus nisi ut. Exercitationem
        saepe dolores, nesciunt quam velit modi maiores magnam facere voluptatum id sit quisquam odio voluptatem?
    </p>
    <div class="flex flex-wrap justify-center gap-6">
        <div class="w-full sm:w-1/2 lg:w-1/4 text-center">
            <div
                class="h-[67px] w-[67px] mx-auto rounded-full flex items-center justify-center text-2xl text-blue-400 shadow-2xl">
                <i class="fa-regular fa-camera"></i>
            </div>
            <h3 class="text-xl">Lorem ipsum dolor sit.</h3>
            <p class="text-slate-500">Lorem ipsum dolor sit amet consectetur adipisicing elit...</p>
        </div>
    </div>
</section>
