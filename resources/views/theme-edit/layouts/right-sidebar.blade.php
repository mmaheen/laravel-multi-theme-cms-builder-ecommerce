<aside
            ...
            <div x-data="{ open: false }" class="rounded-md mb-4 overflow-hidden">
                ...
                <div x-show="open" x-collapse class="p-4">
                    <form action="{{ route('component.update', $productComponent->id) }}" method="POST"
                        ...
                        <input type="number" name="position" value="{{ $productComponent->position }}"
    ...
    <div class="flex-1 overflow-y-auto p-4 space-y-2">
        @foreach ($product->components as $productComponent)
            <form action="{{ route('component.destroy', $productComponent->id) }}" method="POST"
                onsubmit="return confirm('Are you sure you want to delete this component?');">
                @csrf
                @method('DELETE')
                <button type="submit"
                    class="w-full text-left px-4 py-2 bg-red-100 hover:bg-red-200 text-sm text-red-600 rounded-md">
                    <i class="fas fa-trash-alt"></i> Delete {{ ucfirst($productComponent->name) }}
                </button>
            </form>
        @endforeach
    </div>
